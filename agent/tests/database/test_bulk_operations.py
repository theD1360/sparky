...
def test_bulk_add_nodes_missing_fields(repository):
    ...
    nodes = [
        {"node_id": "test:valid", "node_type": "Test", "label": "Valid Node"},
        {
...
def test_bulk_add_edges_success(repository):
    ...
    # First create nodes
    nodes = [
        {"node_id": "test:edge_node1", "node_type": "Test", "label": "Node 1"},
        {"node_id": "test:edge_node2", "node_type": "Test", "label": "Node 2"},
        {"node_id": "test:edge_node3", "node_type": "Test", "label": "Node 3"},
    ]
...
def test_bulk_add_mixed_success_and_failure(repository):
    ...
    nodes = [
        {"node_id": "test:mixed1", "node_type": "Test", "label": "Valid Node 1"},
        {
            "node_id": "test:mixed2",
            # Missing node_type (will fail)
            "label": "Invalid Node",
        },
        {"node_id": "test:mixed3", "node_type": "Test", "label": "Valid Node 3"},
    ]
...
def test_update_node_success(repository):
    ...
    updated = repository.update_node(
        "test:update_test", label="Updated Label", content="Updated content"
    )
...
def test_update_node_partial_update(repository):
    ...
    # Update only content
    updated = repository.update_node("test:partial_update", content="New content only")
...
def test_update_node_all_fields(repository):
    ...
    # Create a node
    repository.add_node("test:update_all", "Test", "Original", "Original content")
...
def test_update_node_properties_replacement(repository):
    ...
    updated = repository.update_node(
        "test:props_replace", properties={"key3": "value3"}
    )
...
def test_update_node_preserves_timestamps(repository):
    ...
    # Create node
    node = repository.add_node("test:timestamps", "Test", "Original")
    ...
    # Update node
    updated = repository.update_node("test:timestamps", label="Updated")
...
def test_append_graph_with_existing_nodes(repository):
    ...
    repository.add_node(
        "concept:existing", "Concept", "Existing Concept", "Already in graph"
    )
    ...
    nodes = [
        ...
        {"node_id": "concept:new", "node_type": "Concept", "label": "New Concept"},
    ]
...
def test_append_graph_nodes_only(repository):
    ...
    nodes = [
        {"node_id": "test:node1", "node_type": "Test", "label": "Node 1"},
        {"node_id": "test:node2", "node_type": "Test", "label": "Node 2"},
    ]
...
def test_append_graph_with_failures(repository):
    ...
    nodes = [
        {"node_id": "test:valid_new", "node_type": "Test", "label": "Valid"},
        {
...
def test_append_graph_complex_structure(repository):
    ...
    edges = [
        ...
        {
            "source_id": "tax:level2_a1",
            "target_id": "tax:level1_a",
            "edge_type": "CHILD_OF",
        },
        {
            "source_id": "tax:level2_a2",
            "target_id": "tax:level1_a",
            "edge_type": "CHILD_OF",
        },
    ]
...
# ============================================================================
# BULK_DELETE_NODES TESTS
# ============================================================================


def test_bulk_delete_nodes_success(repository):
    """Test successful bulk deletion of nodes."""
    # Create nodes
    nodes = [
        {"node_id": "test:delete1", "node_type": "Test", "label": "Delete Node 1"},
        {"node_id": "test:delete2", "node_type": "Test", "label": "Delete Node 2"},
        {"node_id": "test:delete3", "node_type": "Test", "label": "Delete Node 3"},
    ]
    repository.bulk_add_nodes(nodes)

    # Delete all nodes
    result = repository.bulk_delete_nodes(
        ["test:delete1", "test:delete2", "test:delete3"]
    )
    ...
    assert repository.get_node("test:delete1") is None
    assert repository.get_node("test:delete2") is None
    assert repository.get_node("test:delete3") is None


def test_bulk_delete_nodes_with_edges(repository):
    """Test bulk deletion of nodes that have edges."""
    # Create nodes
    nodes = [
        {"node_id": "test:node_with_edges1", "node_type": "Test", "label": "Node 1"},
        {"node_id": "test:node_with_edges2", "node_type": "Test", "label": "Node 2"},
        {"node_id": "test:node_with_edges3", "node_type": "Test", "label": "Node 3"},
    ]
    repository.bulk_add_nodes(nodes)
    ...
    edges = [
        {
            "source_id": "test:node_with_edges1",
            "target_id": "test:node_with_edges2",
            "edge_type": "RELATES_TO",
        },
        {
            "source_id": "test:node_with_edges2",
            "target_id": "test:node_with_edges3",
            "edge_type": "RELATES_TO",
        },
    ]
    repository.bulk_add_edges(edges)

    # Delete nodes with edges
    result = repository.bulk_delete_nodes(
        ["test:node_with_edges1", "test:node_with_edges2"]
    )
    ...
    # Verify nodes and their edges were deleted
    assert repository.get_node("test:node_with_edges1") is None
    assert repository.get_node("test:node_with_edges2") is None
    # Node 3 should still exist
    assert repository.get_node("test:node_with_edges3") is not None


def test_bulk_delete_nodes_not_found(repository):
    """Test bulk deletion with non-existent nodes."""
    # Create only one node
    repository.add_node("test:exists", "Test", "Existing Node")

    result = repository.bulk_delete_nodes(
        ["test:exists", "test:nonexistent1", "test:nonexistent2"]
    )
    ...
    # Verify existing node was deleted
    assert repository.get_node("test:exists") is None

    # Verify not_found list contains the right nodes
    assert "test:nonexistent1" in result["not_found"]
    assert "test:nonexistent2" in result["not_found"]


def test_bulk_delete_nodes_empty_list(repository):
...
def test_bulk_delete_nodes_mixed_results(repository):
    """Test bulk deletion with mix of existing and non-existing nodes."""
    # Create some nodes
    nodes = [
        {"node_id": "test:del_exists1", "node_type": "Test", "label": "Exists 1"},
        {"node_id": "test:del_exists2", "node_type": "Test", "label": "Exists 2"},
    ]
    repository.bulk_add_nodes(nodes)
    ...
    # Verify existing nodes were deleted
    assert repository.get_node("test:del_exists1") is None
    assert repository.get_node("test:del_exists2") is None


def test_bulk_delete_nodes_preserves_other_nodes(repository):
    """Test that bulk delete only removes specified nodes."""
    # Create several nodes
    nodes = [
        {"node_id": "test:keep1", "node_type": "Test", "label": "Keep 1"},
        {"node_id": "test:keep2", "node_type": "Test", "label": "Keep 2"},
        {"node_id": "test:delete_this1", "node_type": "Test", "label": "Delete 1"},
        {"node_id": "test:delete_this2", "node_type": "Test", "label": "Delete 2"},
    ]
    repository.bulk_add_nodes(nodes)

    # Delete only some nodes
    result = repository.bulk_delete_nodes(["test:delete_this1", "test:delete_this2"])

    assert len(result["deleted"]) == 2

    # Verify specified nodes were deleted
    assert repository.get_node("test:delete_this1") is None
    assert repository.get_node("test:delete_this2") is None

    # Verify other nodes are preserved
    assert repository.get_node("test:keep1") is not None
    assert repository.get_node("test:keep2") is not None


def test_bulk_delete_nodes_with_connected_graph(repository):
    """Test bulk deletion in a connected graph structure."""
    # Create a small graph
    nodes = [
        {"node_id": "graph:central", "node_type": "Test", "label": "Central Node"},
        {"node_id": "graph:satellite1", "node_type": "Test", "label": "Satellite 1"},
        {"node_id": "graph:satellite2", "node_type": "Test", "label": "Satellite 2"},
        {"node_id": "graph:satellite3", "node_type": "Test", "label": "Satellite 3"},
    ]
    repository.bulk_add_nodes(nodes)

    edges = [
        {
            "source_id": "graph:satellite1",
            "target_id": "graph:central",
            "edge_type": "CONNECTS_TO",
        },
        {
            "source_id": "graph:satellite2",
            "target_id": "graph:central",
            "edge_type": "CONNECTS_TO",
        },
        {
            "source_id": "graph:satellite3",
            "target_id": "graph:central",
            "edge_type": "CONNECTS_TO",
        },
    ]
    ...
    result = repository.bulk_delete_nodes(["graph:central"])

    assert len(result["deleted"]) == 1
    assert repository.get_node("graph:central") is None

    # Satellite nodes should still exist
    assert repository.get_node("graph:satellite1") is not None
    assert repository.get_node("graph:satellite2") is not None
    assert repository.get_node("graph:satellite3") is not None
