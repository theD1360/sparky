"""Diff-based file editing with tolerance matching.

Ported from wcgw with modifications for Sparky integration.
Provides sophisticated search-replace functionality with:
- Exact and fuzzy matching
- Tolerance for whitespace and indentation differences
- Intelligent indentation fixing
- Multiple match detection
- Edit distance suggestions for failed matches
"""
...
class SearchReplaceMatchError(Exception):
    """Exception raised when search-replace matching fails."""
...
@dataclass
class Tolerance:
    """Configuration for a tolerance level in matching."""
...
class TolerancesHit(Tolerance):
    """Record of tolerance hits during matching."""
...
class FileEditOutput:
    """Result of file editing operation."""
    ...
    def replace_or_throw(
        ...
        """Apply edits or raise exception if errors exceed threshold."""
        new_lines = list[str]()
        last_idx = 0
        errors = []
        warnings = set[str]()
...
---
    ...
    def get_best_match(
        ...
        """Find the best matching edit output(s) based on tolerance scores."""
...
def line_process_max_space_tolerance(line: str) -> str:
    """Remove all spaces for maximum tolerance matching."""
    line = line.strip()
...
def fix_indentation(
    ...
    """Auto-fix indentation in replaced lines based on matched vs searched differences."""
...
def remove_leading_trailing_empty_lines(lines: list[str]) -> list[str]:
    """Remove empty lines from start and end of a line list."""
    start = 0
    end = len(lines) - 1
    if end < start:
        return lines
...
@dataclass
class FileEditInput:
    """Input configuration for file editing operation."""
    ...
    def edit_file(self) -> list[FileEditOutput]:
        """Perform the file edit operation with tolerance matching."""
...
def find_contiguous_match(search_line_positions: list[set[int]]) -> list[slice]:
    """Find contiguous matches of search lines in file."""
...
def match_exact(
    ...
    """Find exact matches of search block in content."""
...
def match_with_tolerance(
    ...
    """Match with tolerance for whitespace and indentation differences."""
    n_search_lines = len(search)
    n_content = len(content) - content_offset
    if n_search_lines > n_content:
        return []
...
def match_with_tolerance_empty_line(
    ...
    """Match with tolerance, ignoring empty lines."""
...
def find_least_edit_distance_substring(
    ...
    """Find the closest match using edit distance for better error messages."""
