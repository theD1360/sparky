"""Search-replace block parsing and editing.

Ported from wcgw with modifications for Sparky integration.
Handles parsing of search-replace blocks with format:
<<<<<<< SEARCH
old content
=======
new content
...
class SearchReplaceSyntaxError(Exception):
    """Exception raised when search-replace block syntax is invalid."""
...
def search_replace_edit(
    ...
    Args:
        lines: Lines containing search-replace blocks
        original_content: Original file content as string
        logger: Function to log matching progress
        
    Returns:
        Tuple of (edited_content, comments)
        
    Raises:
        SearchReplaceSyntaxError: If block syntax is invalid
        SearchReplaceMatchError: If blocks can't be matched
    """
    if not lines:
...
def identify_first_differing_block(
    ...
    This helps provide better error messages when multiple matches are found.
    """
...
def edit_with_individual_fallback(
    ...
    """Apply search-replace blocks with fallback to individual application.
